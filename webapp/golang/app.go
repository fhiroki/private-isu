...
func getPostsID(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT p.id, p.user_id, p.body, p.mime, p.created_at, u.account_name AS `user.account_name` FROM `posts` AS p JOIN `users` AS u ON (p.user_id=u.id) WHERE p.id = ? and u.del_flg=0 ORDER BY p.created_at DESC LIMIT ?"
